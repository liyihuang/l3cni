...
struct bpf_elf_map SEC("maps") bpf_forward_map_controller = {
...
struct bpf_elf_map SEC("maps") proxy_arp_map_controller = {
	.type = BPF_MAP_TYPE_HASH,
	.size_key = sizeof(__u32),
...
int drop_src_dst_ip(struct __sk_buff *skb) {
    ...
    if (eth->h_proto == bpf_htons(ETH_P_ARP)){ 
        ...
        struct reply_mac_if *result = bpf_map_lookup_elem(&proxy_arp_map_controller, &(sip));
        if (result)
    ...
    struct dst_mac_if *result = bpf_map_lookup_elem(&bpf_forward_map_controller, &(ip->daddr));
    if (result) {
